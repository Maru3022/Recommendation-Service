...
class FeedServiceTest {
    ...
    void getFeed_returnsCachedFeedWhenAvailable() throws Exception {
        ...
        when(objectMapper.readValue(anyString(), any())).thenReturn(List.of(postDto));
    ...
    void getFeed_buildsNewFeedWhenCacheMiss() {
        ...
        verify(valueOperations).set(eq("feed:user1:page:0"), anyString(), anyLong(), any());
    }
    ...
    void invalidateCache_deletesUserCacheKeys() {
        when(redisTemplate.keys("feed:user1:*")).thenReturn(Set.of("feed:user1:page:0", "feed:user1:page:1"));

        feedService.invalidateCache("user1");

        verify(redisTemplate).delete(Set.of("feed:user1:page:0", "feed:user1:page:1"));
    }

    @Test
    void invalidateCache_handlesNoKeys() {
        when(redisTemplate.keys("feed:user1:*")).thenReturn(null);
    ...
    void invalidateCache_handlesException() {
        when(redisTemplate.keys("feed:user1:*")).thenThrow(new RuntimeException("Redis error"));
